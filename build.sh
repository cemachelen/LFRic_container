...
# software directory:
APPS_DIR="/nobackup/earhbu/software/apps"
# app information:
APP_NAME='LFRic'
APP_VERSION='20230907'
# build version:
BUILD_VERSION='2'
# top level build dir:
TOP_BUILD_DIR=$(pwd)
...
COMPILER_VERS='gnu:8.3.0'
# mpi libraries for which lfric should be built:
MPI_VERS='mvapich2:2.3.1'
...
do
  ...
  do
    ...
    module load licenses sge ${CMP}/${CMP_VER} ${MP}/${MP_VER} cmake/3.15.1 \
      netcdf hdf5 python3 patchelf svn fcm rose
    ...
    # lfric:
    if [ ! -e ${INSTALL_DIR}/bin/gungho_model ] ; then
      echo "building lfric"
      # set up build dir:
      cd ${BUILD_DIR}
      ...
      cd ${INSTALL_DIR}/miniapps/gungho_model
      # set PROFILE:
      ...
      make -j8 && \
      rsync -aS bin/gungho_model ${INSTALL_DIR}/bin/
      # patch the rpath:
      RPATH_IN=$(patchelf --print-rpath ${INSTALL_DIR}/bin/gungho_model)
      patchelf \
        --set-rpath "${GNU_HOME}/lib64:${NETCDF_HOME}/lib:${HDF5_HOME}/lib:${RPATH_IN}" \
        ${INSTALL_DIR}/bin/gungho_model
      fi
      # lfric:                                                                                                                                                                                                       
      if [ ! -e ${INSTALL_DIR}/bin/lfric_atm ] ; then

      # deactivate python environment:
      cd ${INSTALL_DIR}/lfric_atm
      make clean
     # activate python environment:                                                                                                                                                  
      . ${DEPS_DIR}/python/bin/activate
      # build:                                                                                                                                                                       
      ...
      PSYCLONE_CONFIG="${DEPS_DIR}/python/share/psyclone/psyclone.cfg" \
      make build -j8 && \
      rsync -aS bin/lfric_atm ${INSTALL_DIR}/bin/
      # patch the rpath:                                                                                                                                                             
      RPATH_IN=$(patchelf --print-rpath ${INSTALL_DIR}/bin/lfric_atm)
      patchelf \
        --set-rpath "${GNU_HOME}/lib64:${NETCDF_HOME}/lib:${HDF5_HOME}/lib:${RPATH_IN}" \
        ${INSTALL_DIR}/bin/lfric_atm
